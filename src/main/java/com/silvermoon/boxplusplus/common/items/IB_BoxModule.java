...
public class IB_BoxModule extends ItemBlock {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean p_77624_4_) {
        ...
        list.add(EnumChatFormatting.AQUA.toString() +EnumChatFormatting.BOLD +
                i18n("tile.boxplusplus.boxUI.module.context." + (stack.getItemDamage() + 1)
                    +(update ? "f":"a")));
        if(update){
            list.add(EnumChatFormatting.GOLD+i18n("tile.boxplusplus.boxUI.module."+(stack.getItemDamage()+1))+" (T2)");
        }
        list.add(i18n("tile.boxplusplus.boxUI.module.context."+(stack.getItemDamage()+1)
            +(update?"d":"b")));
    }
